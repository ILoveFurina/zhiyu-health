# 智愈 server-java 与 server-py 系统分析与设计（模板版）
...
- [11 API 设计](#11-api-设计)
  - [11.1 全局协议](#111-全局协议)
  - [11.2 C 端 API](#112-c-端-api)
  - [11.3 B 端 API](#113-b-端-api)
  - [11.4 Agent 工具回调 API（server-py → server-java）](#114-agent-工具回调-apiserver-py--server-java)
  - [11.5 server-py 服务 API（server-java → server-py）](#115-server-py-服务-apiserver-java--server-py)
  - [11.6 核心契约示例](#116-核心契约示例)
  - [11.7 实时事件契约](#117-实时事件契约)
- [12 关键技术设计](#12-关键技术设计)
...
| 2026-07-31 | v3.0 | 结构优化：修复图例与判断节点错误、去重、字段字典移附录、补待确认标记 | 项目组 |
...
> 【待确认】此为 demo 阶段性拓扑还是目标生产架构？

### 7.2 组件与依赖关系

上述分层与边界的组件级调用关系及存储读写权限见下图。边标注了认证方式与读写权限；实线为常规业务调用链，虚线为端侧入口接入（C/B 端 → controller）与规则引擎对 Neo4j 的旁路只读查询。
...
上图突出状态对象；完整业务实体还包括 Hospital、Department、StaffUser、Medication、ConsultationRecord、HealthProfileAllergy、KnowledgeChunk 与 InAppMessage，其关系由下方 ER 图给出。状态值一律来自 `schema.sql` CHECK 或 `contracts/`，不得在调用端自行新增。

### 7.4 业务实体 ER 图

实体仅列主键与核心字段（完整定义见「附录 A 字段字典」）；`FK` 标注外键来源，`o|--` 表示左端可为零（即外键可空）。
...
flowchart TD
    ...
    C -- 否 --> E{"命中红线?"}
    E -- 命中 --> F1["创建会话/用户消息/ChatRound"]
    F1 --> G["持久化红线结果，轮次直接 COMPLETED"]
    E -- 未命中 --> F2["创建会话/用户消息/ChatRound"]
    F2 --> H["轮次独立运行"]
...
- 断连重连的“补发”只针对事件流：运行中的轮次把已发出的事件重新推给新连接，轮次本身只执行一次。
...
flowchart TD
    ...
    I -- 成功 --> J["server-java 持久化结果"]
    I -- 输入非法/模型超时/输出校验失败 --> X["server-java 落 FAILED + 白名单错误码（422/504/502）"]
    J --> K["返回报告解读卡片 + 免责声明"]
    X --> Y["返回失败卡片（FAILED + error_code）"]
```
...
下图覆盖四类分支：已存在轮次的幂等重放（已完成回放、运行中补发、进程重启标记失败）、命中红线的自闭环、正常 Agent 流（含 RAG 降级与最终 `message`）。FAILED 路径在末尾给出。
...
sequenceDiagram
    ...
    alt 已存在轮次（幂等重放）
      ...
      else 状态为 ACCEPTED/RUNNING 且进程内有运行态
        J-->>C: 重放已落库事件（meta/knowledge/token/卡片）
        Note over J: 轮次不重跑，新连接继续接收后续实时事件
...
本流程跨 server-java、Neo4j、server-py 三方，覆盖医生开方提交与管理员审核两个阶段；关键设计要点见末尾说明。
...
完整字段字典（各表字段、类型、可空与默认值、约束与索引）见 [附录 A 字段字典](#附录-a-字段字典)。
...
本章各接口表为联调契约；响应包装与错误形状统一遵循 11.1 全局协议。
...
**响应**：`token`、`patient:{id,nickname}`
**幂等与错误**：同 nickname 复用患者；超长 400
...
**响应**：SSE 事件流，见“实时事件契约”
**幂等与错误**：`(patient_id,request_id)` 幂等；409 当前会话有运行轮次；命中红线规则不调用 Agent
...
**响应**：列表 `{id,title,last_active_at}`；消息列表 `{id,role,kind,content,effort,disclaimer,created_at}`
**幂等与错误**：404 不存在/不属于患者；DELETE 返回 204
...
**响应**：`{id,display_name,gender,birth_date,relationship,active,allergies,created_at}`；`/current` 的 `profile` 为 object 或 null
**幂等与错误**：POST 201；激活与替换过敏史按患者归属，404 防枚举；激活事务幂等

##### 健康时间线 `GET /api/c/health-profiles/{id}/timeline`

**响应**：`[{record_id,type,title,summary,occurred_at,disclaimer}]`
**幂等与错误**：404 不属于患者

##### 挂号 `GET /api/c/appointments`、`POST /api/c/appointments/{id}/cancel`

**响应**：`[{appointment_id,schedule_id,doctor_id,doctor_name,department_name,schedule_date,time_slot,sequence_number,status,condition_summary,summary_disclaimer,created_at}]`
**幂等与错误**：取消重复调用幂等；非 BOOKED 409；不存在/不归属 404
...
**响应**：`{report_interpretation_id,conversation_id,status,result,page_count,error_code,disclaimer}`
**幂等与错误**：`(patient_id,request_id)` 幂等；类型/大小/数量按 contracts；模型错误使用 vision 白名单码
...
**响应**：上传 `{uploaded,total,ready}`；finalize 同报告直传响应
**幂等与错误**：`(patient_id,request_id)` 隔离；非法页/类型/大小返回 422，批次数量不一致或 finalize 缺页返回 409；take/finalize 后清理暂存

##### 患者处方 `GET /api/c/prescriptions`

**响应**：仅当前激活档案的 `APPROVED` 列表：处方头、items、interpretation、disclaimer
**幂等与错误**：无激活档案 409；不返回 PENDING/REJECTED

##### 站内消息 `GET /api/c/messages`

**响应**：`[{id,type,title,content,disclaimer,created_at}]`
...
**响应**：`{access_token,token_type:"bearer"}`；`/me` 返回 `{username,role,doctor_id}`
**状态码与错误**：凭据错误 401；不得区分账号不存在/密码错误
...
**响应**：`{id,name,level,address,longitude,latitude}`
**状态码与错误**：POST 201；重名/被引用 409；仅 admin
...
**响应**：科室实体及 hospital 关联字段
**状态码与错误**：hospital 不存在 400/404；被医生引用时删除 409
...
**响应**：医生实体及科室信息
**状态码与错误**：department 不存在 400/404；被排班/员工引用时删除 409
...
**响应**：`{id,doctor_id,schedule_date,time_slot,total_slots,remaining_slots,is_active}`
**状态码与错误**：POST 201；创建/更新 DTO 不接收 `is_active`；更新容量按 delta 原子调整；停用幂等；有挂号时删除/非法缩容 409
...
**响应**：今日排班/挂号详情；完成后 `status=VISITED`、接诊记录、可选关怀消息
**状态码与错误**：仅绑定 doctor；非本人挂号 404；取消/重复完成 409
...
**响应**：`{decision,message_type,blocked,reasons,message,advice}`
**状态码与错误**：仅 doctor；Neo4j 不可用/事实不全返回 REVIEW_REQUIRED 且 blocked=true
...
**响应**：`PrescriptionView{id,appointment_id,status,notes,...,items}`
**状态码与错误**：提交时强制复检；已开方/非 SAFE 409；同 appointment 唯一约束兜底
...
**响应**：更新后的 `PrescriptionView`
**状态码与错误**：REJECT reason 必填；条件更新保证并发仅一次成功，后到请求 409；解读生成失败保持 PENDING

### 11.4 Agent 工具回调 API（server-py → server-java）
...
| POST | `/api/agent/appointments/{id}/summary` | save summary（非模型直接入口） | 供 server-py 在工具流程之外补写病情摘要 |

工具执行异常被 server-py 规整为模型可解释文本，不投影成成功卡片；售罄、参数臆造和 server-java 暂不可用不应直接掐断整条 Agent 流。

| 工具契约 | 参数 | 返回 |
| --- | --- | --- |
| recommend_doctors | query `department_name*:string` | `{doctors:[{doctor_id,name,title,specialty,photo_url,remaining_slots}]}` |
| get_doctor_slots | path `doctor_id*:positive` | `{doctor_id,slots:[{schedule_id,schedule_date,time_slot,remaining_slots}]}` |
| find_hospitals | query `longitude*,latitude*`，坐标来自可信 AgentContext | `{hospitals:[{hospital_id,name,level,address,distance_km}]}` |
| create_appointment | JSON `patient_id*,conversation_id*,schedule_id*,condition_summary*` | AppointmentCard，含 `summary_sent/summary_disclaimer/notice` |
...
### 11.5 server-py 服务 API（server-java → server-py）
...
| POST | `/api/agent/clinical/prescription-explanation` | items | content + disclaimer | Agent callback secret |
...
> 【待确认】`high` 对应的具体 API 参数形态（原文仅写 `high`）。
...
| Agent 超时 | 对话轮次总超时 300s（报告解读视觉调用另有专用约束，见 9.4）；模型/工具子调用采用更短超时且不无限重试副作用工具 | 超时测试桩 |
...
> 【待补充】本节需补充以下风险与外部依赖分析：
>
> - 火山方舟 LLM 的配额、限流与供应商不可用应对；
> - 支付宝平台依赖（小程序审核、平台能力变更等）；
> - 生产部署形态：7.1 的“应用不部署上云”是 demo 阶段性拓扑还是目标架构；
> - C 端消息/会话列表接口的分页策略（13.3 容量基线按消息 100 万条估算，而 11.2 的 GET messages 为全量返回）。
...
> 【待补充】里程碑日期、并行工作线安排与 buffer；“测试与联调 1 天”偏乐观。
