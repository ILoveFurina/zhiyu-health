...
4. **约束只拦叫号**：完成接诊（`complete`）与查看处方不拦时段。配合票 87 另一条硬约束--`complete` 废弃 `BOOKED -> VISITED` 直通、只接受 `IN_PROGRESS -> VISITED`（见 ADR-0033 修订），叫号成为进入就诊中的唯一入口，跳过叫号无法完成接诊，时段约束因此在叫号这一环闭环。
