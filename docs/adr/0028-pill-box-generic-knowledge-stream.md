...
Status: accepted（票 51 拍药盒收口与说明书流）；部分取代 ADR-0025
...
vision 管道仍只提候选药名（ADR-0025 保留部分），之后不再查 `medications` 业务表、不读健康档案、不调规则引擎、不查 Neo4j 禁忌事实。客户端拿到药名后经 realtime 通道发送 `medication_name` 信封，server-java 透传至 server-py `/api/agent/medication/knowledge`，LLM 按通用语料流式输出说明书（用途/常规用法用量/常见不良反应/常见注意事项四节），经 SSE/WS 逐 token 回落会话 `KIND_TEXT` 消息。说明书不绑业务表、不引联网搜索、不做药房联动。
...
- server-py 新增 `/api/agent/medication/knowledge`：prompt 四节边界，不接收任何患者档案字段，禁止个性化剂量与替代药建议，不认识的药输出 `unknown_drug` 话术。
