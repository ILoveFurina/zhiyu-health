...
public class RedFlagRuleEngine {
    ...
    private static final List<Rule> RULES = List.of(
            ...
            new Rule("急性中毒", List.of(List.of(
                    "服毒", "农药中毒", "喝了农药", "服了农药", "误服农药", "煤气中毒", "一氧化碳中毒")))
    );

    public RedFlagHit judge(String text) {
        String compact = text.replaceAll("\\s+", "");
        return RULES.stream()
                .filter(rule -> rule.groups().stream()
                        .allMatch(group -> group.stream().anyMatch(compact::contains)))
                .findFirst()
