...
public class ReportInterpretationService {
    ...
    public ReportInterpretationService(ReportInterpretationPersistence persistence,
                                       AgentClient agentClient, ObjectMapper objectMapper,
    ...
    public ReportView interpret(Long patientId, Long conversationId, String requestId,
    ...
    private void validate(List<MultipartFile> files, String requestId) {
        ...
        for (MultipartFile file : files) {
            total += file.getSize();
            if (file.isEmpty() || file.getSize() > TEN_MB
                    || (!pdf && !IMAGE_TYPES.contains(file.getContentType()))) {
    ...
    private String contextSummary(JsonNode result) {
        StringBuilder summary = new StringBuilder("报告解读：")
                .append(result.path("summary").asText());
        for (JsonNode item : result.path("items")) {
            summary.append("；").append(item.path("name").asText())
                    .append(" ").append(item.path("value").asText())
                    .append("（参考 ").append(item.path("reference_range").asText()).append("）")
                    .append("，关注级别 ").append(item.path("priority").asText());
