...
public class ReportInterpretationPersistence {
    ...
    public ReportInterpretationPersistence(ReportInterpretationMapper mapper,
    ...
    @Transactional
    public ReportInterpretation start(Long patientId, Long conversationId, String requestId,
                                      List<MultipartFile> files) {
        Conversation conversation = conversations.getOrCreateForPatient(
                patientId, conversationId, "看报告");
        ...
        conversations.appendMessage(
                conversation.getId(), "user", upload.toString(),
                Message.KIND_REPORT_UPLOAD, null, record.getId());
        return record;
    }

    @Transactional
    public ReportInterpretation succeed(ReportInterpretation record,
                                        AgentClient.VisionResponse response,
                                        String resultJson, String contextSummary) {
        ...
        conversations.appendMessage(
                record.getConversationId(), "assistant", card.toString(),
                Message.KIND_REPORT_INTERPRETATION, null, record.getId());
        conversations.appendMessage(
                record.getConversationId(), "assistant", contextSummary,
                Message.KIND_REPORT_CONTEXT, null, record.getId());
        return record;
    }
