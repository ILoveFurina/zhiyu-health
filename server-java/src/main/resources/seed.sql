...
-- is_prescription（票 74，ADR-0032）：处方药 TRUE 须凭已审核处方购药，OTC FALSE 可由 C 端 AI 购药直接下单。
-- 抗生素/抗凝/心血管/降糖/PPI/呼吸处方/甲状腺类标 TRUE；常见 OTC 解热镇痛/抗过敏/止泻/促胃动力/维生素钙标 FALSE。
-- 票 88（ADR-0035）：medications 收敛为标准药品目录，价格/库存/在售移至下方 pharmacy_medications（按药房独立）。
INSERT INTO medications (id, name, generic_name, specification, instructions, is_prescription) VALUES
    (1, '阿莫西林胶囊', '阿莫西林', '0.25g*24粒', '口服；青霉素过敏者禁用', TRUE),
    (2, '布洛芬缓释胶囊', '布洛芬', '0.3g*20粒', '口服；建议餐后服用', FALSE),
    (3, '氯雷他定片', '氯雷他定', '10mg*12片', '口服；每日一次', FALSE),
    (4, '华法林钠片', '华法林钠', '2.5mg*60片', '口服；须遵医嘱监测凝血指标', TRUE),
    (5, '对乙酰氨基酚片', '对乙酰氨基酚', '0.5g*20片', '口服；按医嘱使用，不得超量', FALSE),
    (6, '头孢克洛胶囊', '头孢克洛', '0.25g*12粒', '口服；头孢菌素过敏者禁用', TRUE),
    (7, '阿奇霉素片', '阿奇霉素', '0.25g*6片', '口服；按医嘱完成疗程', TRUE),
    (8, '左氧氟沙星片', '左氧氟沙星', '0.5g*5片', '口服；喹诺酮类过敏者禁用', TRUE),
    (9, '奥美拉唑肠溶胶囊', '奥美拉唑', '20mg*14粒', '口服；通常餐前服用', TRUE),
    (10, '二甲双胍片', '二甲双胍', '0.5g*20片', '口服；建议随餐服用', TRUE),
    (11, '阿卡波糖片', '阿卡波糖', '50mg*30片', '口服；与第一口主食同服', TRUE),
    (12, '氨氯地平片', '苯磺酸氨氯地平', '5mg*14片', '口服；按医嘱监测血压', TRUE),
    (13, '厄贝沙坦片', '厄贝沙坦', '150mg*7片', '口服；按医嘱监测血压', TRUE),
    (14, '美托洛尔缓释片', '琥珀酸美托洛尔', '47.5mg*7片', '口服；不可自行突然停药', TRUE),
    (15, '阿托伐他汀钙片', '阿托伐他汀钙', '20mg*7片', '口服；按医嘱监测肝功能', TRUE),
    (16, '瑞舒伐他汀钙片', '瑞舒伐他汀钙', '10mg*7片', '口服；按医嘱监测肝功能', TRUE),
    (17, '阿司匹林肠溶片', '阿司匹林', '100mg*30片', '口服；有出血风险者须遵医嘱', TRUE),
    (18, '氯吡格雷片', '硫酸氢氯吡格雷', '75mg*7片', '口服；有出血风险者须遵医嘱', TRUE),
    (19, '蒙脱石散', '蒙脱石', '3g*10袋', '口服；与其他药物错开服用', FALSE),
    (20, '多潘立酮片', '多潘立酮', '10mg*30片', '口服；通常餐前服用', FALSE),
    (21, '西替利嗪片', '盐酸西替利嗪', '10mg*12片', '口服；每日一次', FALSE),
    (22, '孟鲁司特钠片', '孟鲁司特钠', '10mg*5片', '口服；按医嘱使用', TRUE),
    (23, '沙丁胺醇吸入气雾剂', '硫酸沙丁胺醇', '100μg*200揿', '吸入；按医嘱使用', TRUE),
    (24, '布地奈德吸入粉雾剂', '布地奈德', '200μg*100吸', '吸入；使用后漱口', TRUE),
    (25, '左甲状腺素钠片', '左甲状腺素钠', '50μg*100片', '口服；通常空腹服用', TRUE),
    (26, '甲巯咪唑片', '甲巯咪唑', '5mg*100片', '口服；按医嘱监测血常规', TRUE),
    (27, '碳酸钙D3片', '碳酸钙D3', '600mg*30片', '口服；与部分药物错开服用', FALSE),
    (28, '维生素B2片', '核黄素', '5mg*100片', '口服；按医嘱使用', FALSE),
    (29, '甲硝唑片', '甲硝唑', '0.2g*20片', '口服；用药期间及停药后避免饮酒', TRUE),
    (30, '呋喃妥因肠溶片', '呋喃妥因', '50mg*100片', '口服；硝基呋喃类过敏者禁用', TRUE)
ON CONFLICT (id) DO NOTHING;

-- 院区药房（票 88，ADR-0035）：每个院区恰好一个，与院区强一对一；id 与院区 id 同值便于演示对照。
-- 展示名/配送费/预计配送分钟数均为虚构演示数据（即「模拟配送报价」，下单时固化为订单快照）。
INSERT INTO campus_pharmacies (id, campus_id, display_name, delivery_fee, estimated_delivery_minutes) VALUES
    (11, 11, '主院区药房', 5.00, 45),
    (12, 12, '郑东院区药房', 6.00, 60),
    (13, 13, '南院区药房', 4.00, 40),
    (21, 21, '西院区药房', 5.50, 50),
    (22, 22, '东院区药房', 6.50, 65),
    (31, 31, '主院区药房', 3.50, 35)
...
-- 药房药品（票 88，ADR-0035）：各院区药房从 30 个标准药品中独立选品，价格/库存各异、大部分在售。
-- 确定性伪随机分布（同一药房同一药品幂等）：每药房约 25 种在售关系，共 150 行（verify 基线）。
INSERT INTO pharmacy_medications (id, pharmacy_id, medication_id, price, stock, is_on_sale) VALUES
    (1, 11, 1, 15.50, 53, TRUE),
    (2, 11, 2, 22.50, 66, TRUE),
    (3, 11, 3, 29.50, 79, TRUE),
    (4, 11, 4, 36.50, 92, TRUE),
    (5, 11, 5, 43.50, 105, TRUE),
    (6, 11, 7, 57.50, 131, TRUE),
    (7, 11, 8, 9.50, 144, FALSE),
    (8, 11, 9, 16.50, 157, TRUE),
    (9, 11, 10, 23.50, 170, TRUE),
    (10, 11, 11, 30.50, 183, TRUE),
    (11, 11, 13, 44.50, 209, TRUE),
    (12, 11, 14, 51.50, 222, TRUE),
    (13, 11, 15, 58.50, 235, TRUE),
    (14, 11, 16, 10.50, 248, FALSE),
    (15, 11, 17, 17.50, 261, TRUE),
    (16, 11, 19, 31.50, 287, TRUE),
    (17, 11, 20, 38.50, 300, TRUE),
    (18, 11, 21, 45.50, 313, TRUE),
    (19, 11, 22, 52.50, 326, TRUE),
    (20, 11, 23, 59.50, 339, TRUE),
    (21, 11, 25, 18.50, 365, TRUE),
    (22, 11, 26, 25.50, 378, TRUE),
    (23, 11, 27, 32.50, 391, TRUE),
    (24, 11, 28, 39.50, 44, TRUE),
    (25, 11, 29, 46.50, 57, TRUE),
    (26, 12, 1, 26.50, 70, TRUE),
    (27, 12, 2, 33.50, 83, TRUE),
    (28, 12, 3, 40.50, 96, TRUE),
    (29, 12, 4, 47.50, 109, TRUE),
    (30, 12, 6, 61.50, 135, TRUE),
    (31, 12, 7, 13.50, 148, TRUE),
    (32, 12, 8, 20.50, 161, TRUE),
    (33, 12, 9, 27.50, 174, TRUE),
    (34, 12, 10, 34.50, 187, TRUE),
    (35, 12, 12, 48.50, 213, TRUE),
    (36, 12, 13, 55.50, 226, FALSE),
    (37, 12, 14, 62.50, 239, TRUE),
    (38, 12, 15, 14.50, 252, TRUE),
    (39, 12, 16, 21.50, 265, TRUE),
    (40, 12, 18, 35.50, 291, TRUE),
    (41, 12, 19, 42.50, 304, TRUE),
    (42, 12, 20, 49.50, 317, TRUE),
    (43, 12, 21, 56.50, 330, FALSE),
    (44, 12, 22, 8.50, 343, TRUE),
    (45, 12, 24, 22.50, 369, TRUE),
    (46, 12, 25, 29.50, 382, TRUE),
    (47, 12, 26, 36.50, 395, TRUE),
    (48, 12, 27, 43.50, 48, TRUE),
    (49, 12, 28, 50.50, 61, TRUE),
    (50, 12, 30, 9.50, 87, TRUE),
    (51, 13, 1, 37.50, 87, TRUE),
    (52, 13, 2, 44.50, 100, FALSE),
    (53, 13, 3, 51.50, 113, TRUE),
    (54, 13, 5, 10.50, 139, TRUE),
    (55, 13, 6, 17.50, 152, TRUE),
    (56, 13, 7, 24.50, 165, TRUE),
    (57, 13, 8, 31.50, 178, TRUE),
    (58, 13, 9, 38.50, 191, TRUE),
    (59, 13, 11, 52.50, 217, TRUE),
    (60, 13, 12, 59.50, 230, TRUE),
    (61, 13, 13, 11.50, 243, TRUE),
    (62, 13, 14, 18.50, 256, TRUE),
    (63, 13, 15, 25.50, 269, TRUE),
    (64, 13, 17, 39.50, 295, TRUE),
    (65, 13, 18, 46.50, 308, FALSE),
    (66, 13, 19, 53.50, 321, TRUE),
    (67, 13, 20, 60.50, 334, TRUE),
    (68, 13, 21, 12.50, 347, TRUE),
    (69, 13, 23, 26.50, 373, TRUE),
    (70, 13, 24, 33.50, 386, TRUE),
    (71, 13, 25, 40.50, 399, TRUE),
    (72, 13, 26, 47.50, 52, FALSE),
    (73, 13, 27, 54.50, 65, TRUE),
    (74, 13, 29, 13.50, 91, TRUE),
    (75, 13, 30, 20.50, 104, TRUE),
    (76, 21, 1, 48.50, 104, TRUE),
    (77, 21, 2, 55.50, 117, TRUE),
    (78, 21, 4, 14.50, 143, TRUE),
    (79, 21, 5, 21.50, 156, TRUE),
    (80, 21, 6, 28.50, 169, TRUE),
    (81, 21, 7, 35.50, 182, FALSE),
    (82, 21, 8, 42.50, 195, TRUE),
    (83, 21, 10, 56.50, 221, TRUE),
    (84, 21, 11, 8.50, 234, TRUE),
    (85, 21, 12, 15.50, 247, TRUE),
    (86, 21, 13, 22.50, 260, TRUE),
    (87, 21, 14, 29.50, 273, TRUE),
    (88, 21, 16, 43.50, 299, TRUE),
    (89, 21, 17, 50.50, 312, TRUE),
    (90, 21, 18, 57.50, 325, TRUE),
    (91, 21, 19, 9.50, 338, TRUE),
    (92, 21, 20, 16.50, 351, TRUE),
    (93, 21, 22, 30.50, 377, TRUE),
    (94, 21, 23, 37.50, 390, FALSE),
    (95, 21, 24, 44.50, 43, TRUE),
    (96, 21, 25, 51.50, 56, TRUE),
    (97, 21, 26, 58.50, 69, TRUE),
    (98, 21, 28, 17.50, 95, TRUE),
    (99, 21, 29, 24.50, 108, TRUE),
    (100, 21, 30, 31.50, 121, TRUE),
    (101, 22, 1, 59.50, 121, TRUE),
    (102, 22, 3, 18.50, 147, TRUE),
    (103, 22, 4, 25.50, 160, FALSE),
    (104, 22, 5, 32.50, 173, TRUE),
    (105, 22, 6, 39.50, 186, TRUE),
    (106, 22, 7, 46.50, 199, TRUE),
    (107, 22, 9, 60.50, 225, TRUE),
    (108, 22, 10, 12.50, 238, TRUE),
    (109, 22, 11, 19.50, 251, TRUE),
    (110, 22, 12, 26.50, 264, FALSE),
    (111, 22, 13, 33.50, 277, TRUE),
    (112, 22, 15, 47.50, 303, TRUE),
    (113, 22, 16, 54.50, 316, TRUE),
    (114, 22, 17, 61.50, 329, TRUE),
    (115, 22, 18, 13.50, 342, TRUE),
    (116, 22, 19, 20.50, 355, TRUE),
    (117, 22, 21, 34.50, 381, TRUE),
    (118, 22, 22, 41.50, 394, TRUE),
    (119, 22, 23, 48.50, 47, TRUE),
    (120, 22, 24, 55.50, 60, TRUE),
    (121, 22, 25, 62.50, 73, TRUE),
    (122, 22, 27, 21.50, 99, TRUE),
    (123, 22, 28, 28.50, 112, FALSE),
    (124, 22, 29, 35.50, 125, TRUE),
    (125, 22, 30, 42.50, 138, TRUE),
    (126, 31, 2, 22.50, 151, TRUE),
    (127, 31, 3, 29.50, 164, TRUE),
    (128, 31, 4, 36.50, 177, TRUE),
    (129, 31, 5, 43.50, 190, TRUE),
    (130, 31, 6, 50.50, 203, TRUE),
    (131, 31, 8, 9.50, 229, TRUE),
    (132, 31, 9, 16.50, 242, FALSE),
    (133, 31, 10, 23.50, 255, TRUE),
    (134, 31, 11, 30.50, 268, TRUE),
    (135, 31, 12, 37.50, 281, TRUE),
    (136, 31, 14, 51.50, 307, TRUE),
    (137, 31, 15, 58.50, 320, TRUE),
    (138, 31, 16, 10.50, 333, TRUE),
    (139, 31, 17, 17.50, 346, FALSE),
    (140, 31, 18, 24.50, 359, TRUE),
    (141, 31, 20, 38.50, 385, TRUE),
    (142, 31, 21, 45.50, 398, TRUE),
    (143, 31, 22, 52.50, 51, TRUE),
    (144, 31, 23, 59.50, 64, TRUE),
    (145, 31, 24, 11.50, 77, TRUE),
    (146, 31, 26, 25.50, 103, TRUE),
    (147, 31, 27, 32.50, 116, TRUE),
    (148, 31, 28, 39.50, 129, TRUE),
    (149, 31, 29, 46.50, 142, TRUE),
    (150, 31, 30, 53.50, 155, TRUE)
ON CONFLICT (id) DO NOTHING;
...
-- 票 88：模板明细补配药数量（正整数，引用模板开方时带入处方明细）。
INSERT INTO prescription_template_items (id, template_id, medication_id, dosage, frequency, duration, quantity, notes) VALUES
    (1, 1, 12, '5mg', '每日1次', '30天', 2, '晨起服用，按医嘱监测血压'),
    (2, 1, 13, '150mg', '每日1次', '30天', 4, '按医嘱监测血压'),
    (3, 2, 17, '100mg', '每日1次', '30天', 1, '餐后服用，有出血风险者须遵医嘱'),
    (4, 2, 15, '20mg', '每晚1次', '30天', 4, '按医嘱监测肝功能'),
    (5, 3, 17, '100mg', '每日1次', '14天', 1, '有出血风险者须遵医嘱'),
    (6, 3, 16, '10mg', '每晚1次', '14天', 2, '按医嘱监测肝功能')
ON CONFLICT (id) DO NOTHING;
...
SELECT setval('campus_pharmacies_id_seq', (SELECT MAX(id) FROM campus_pharmacies));
SELECT setval('pharmacy_medications_id_seq', (SELECT MAX(id) FROM pharmacy_medications));
