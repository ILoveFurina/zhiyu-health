...
-- 医院科室分类（票 49）：一家医院为组织和展示本院科室而维护的分类及顺序，
-- 各院区共享该医院的分类体系；分类不是平台统一枚举，不参与 Agent 医学判断。
CREATE TABLE IF NOT EXISTS department_categories (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    hospital_id BIGINT NOT NULL REFERENCES hospitals(id),
    name VARCHAR(50) NOT NULL,
    sort_order INT NOT NULL DEFAULT 0,
    CONSTRAINT uq_department_categories_hospital_name UNIQUE (hospital_id, name)
);

-- 实际科室：归属具体院区，并归入所属医院的科室分类（票 49）。
-- category 必须与 campus 同属一家医院（由 server-java service 校验，DB 不跨表约束）。
...
CREATE INDEX IF NOT EXISTS idx_department_categories_hospital ON department_categories(hospital_id);
...
-- department_categories 医院科室分类
COMMENT ON TABLE department_categories IS '医院科室分类表：一家医院为组织和展示本院科室而维护的分类，各院区共享';
COMMENT ON COLUMN department_categories.id IS '主键，BIGINT 标识列';
COMMENT ON COLUMN department_categories.hospital_id IS '所属医院 ID，外键 hospitals(id)，限制删除';
COMMENT ON COLUMN department_categories.name IS '分类名称，同一医院内唯一';
...
COMMENT ON COLUMN departments.category_id IS '所属医院科室分类 ID，外键 department_categories(id)，限制删除（必须与院区同医院）';
