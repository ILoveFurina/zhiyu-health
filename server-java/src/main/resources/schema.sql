...
-- 接诊记录来源二选一（票 55）：挂号单或在线问诊，两个真实外键恰好一个非空
...
-- 处方来源二选一（票 55）：与 consultation_records 同一约定，禁止 source_type/source_id 多态列。
...
-- =============================================================================
-- 票 54 在线问诊主闭环（Spec 0003）：预问诊草稿、在线问诊单、医患消息。
...
-- 票 55：处方/接诊记录来源泛化为挂号单或在线问诊二选一（两个真实外键，禁止多态列）。
...
COMMENT ON COLUMN consultation_records.appointment_id IS '关联预约 ID，外键 appointments(id)，唯一；与 online_consultation_id 二选一（票 55）';
COMMENT ON COLUMN consultation_records.online_consultation_id IS '关联在线问诊 ID，外键 online_consultations(id)，唯一；与 appointment_id 二选一（票 55）';
...
COMMENT ON COLUMN prescriptions.appointment_id IS '关联预约 ID，外键 appointments(id)，唯一；与 online_consultation_id 二选一（票 55）';
COMMENT ON COLUMN prescriptions.online_consultation_id IS '关联在线问诊 ID，外键 online_consultations(id)，唯一；与 appointment_id 二选一（票 55）';
...
-- preconsultation_drafts 预问诊草稿（票 54）
...
-- online_consultations 在线问诊单（票 54）
...
-- online_consultation_messages 医患消息（票 54）
