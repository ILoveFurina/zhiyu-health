...
class DoctorRecommendationControllerTest {
    ...
    void returnsAvailableSlotsForDoctor() throws Exception {
        when(recommendationService.getDoctorSlots(2L))
                .thenReturn(List.of(new DoctorRecommendationService.DoctorSlot(
                        9L, LocalDate.of(2026, 7, 29), TimeSlot.MORNING, 3)));

        mockMvc.perform(get("/api/agent/doctors/2/slots").header(AgentCallbackAuthFilter.HEADER_NAME, CALLBACK_SECRET))
                .andExpect(status().isOk())
