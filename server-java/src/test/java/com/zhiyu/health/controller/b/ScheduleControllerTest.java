...
class ScheduleControllerTest {
    ...
    void adminCanListAndReadSchedules() throws Exception {
        ...
        mockMvc.perform(get("/api/b/schedules").with(StaffTokens.withRole(StaffUser.ROLE_ADMIN)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].remaining_slots").value(12));
        mockMvc.perform(get("/api/b/schedules/1").with(StaffTokens.withRole(StaffUser.ROLE_ADMIN)))
                .andExpect(status().isOk())
    ...
    void adminCanDisableAndDeleteWithoutRemovingSlotPool() throws Exception {
        ...
        mockMvc.perform(patch("/api/b/schedules/1/disable").with(StaffTokens.withRole(StaffUser.ROLE_ADMIN)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.is_active").value(false));
        mockMvc.perform(delete("/api/b/schedules/1").with(StaffTokens.withRole(StaffUser.ROLE_ADMIN)))
    ...
    void missingScheduleReturns404() throws Exception {
        ...
        mockMvc.perform(get("/api/b/schedules/99").with(StaffTokens.withRole(StaffUser.ROLE_ADMIN)))
    ...
    void doctorCannotManageSchedules() throws Exception {
        mockMvc.perform(get("/api/b/schedules").with(StaffTokens.withRole(StaffUser.ROLE_DOCTOR)))
