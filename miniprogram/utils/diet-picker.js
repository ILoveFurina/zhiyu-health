const CONSENT_KEY = 'diet_ai_consent_v1'
...
function confirmConsent() {
  ...
  return new Promise((resolve, reject) => {
    my.confirm({
      title: '饮食拍照分析说明',
      content: '请确认你有权上传该照片。内容将发送至火山方舟多模态模型处理，照片原图会留存于你的历史会话中供回看。仅供参考，不替代医生诊断，特殊饮食需求请咨询医生或营养师。',
...
async function chooseDietPhoto(index) {
  await confirmConsent()
  // 饮食场景只接受图片（无 PDF），0=拍摄，1=相册
  return chooseImages(index === 0 ? 'camera' : 'album')
}

module.exports = { chooseDietPhoto }
