...
function parseResponse(res) {
  ...
  const message = (data && data.message) || (typeof detail === 'string' && detail)
    || (detail && detail.message) || `皮肤分析失败（${status}）`
...
function uploadSkinPhoto({ requestId, conversationId, item }) {
  ...
  return new Promise((resolve, reject) => {
    my.uploadFile({
      ...
      fail: () => reject(new Error('无法上传皮肤照片，请检查网络')),
    })
  })
}

module.exports = { uploadSkinPhoto }
