...
async def asr(
    ...
    try:
        ...
        raise HTTPException(
            status_code=_status_for(exc.code), detail={"code": exc.code}
        ) from exc
    except TimeoutError as exc:
        raise HTTPException(
            status_code=504, detail={"code": "VOICE_MODEL_TIMEOUT"}
        ) from exc
...
@router.post("/tts")
async def tts(
    body: TtsRequest, request: Request, _: AgentCallbackAuth
) -> Response:
    contract = get_contracts().voice
    try:
