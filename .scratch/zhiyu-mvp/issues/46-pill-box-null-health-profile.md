...
- 2026-08-06（小程序补记）：小程序实测发现“上传照片后只见照片气泡、无任何结果/引导文字”。根因：`finishPillbox`/`finishMedLookup` 的 not_found 分支注释称“仅刷新会话”但从未实现，且 `MedicationLookupView` 响应无 hint 字段，后端已落库的引导文案（未识别药名/未匹配到药）前端拿不到。修复：①`MedicationLookupView` 增 `hint`/`reminder` 字段，notFound 响应携带 hint，空过敏史成功响应携带 `reminder` 并追加一条 KIND_TEXT 助手消息（contracts/contraindication.json 新增 `remind_profile`：“你还没有在健康档案中填写过敏史，建议完善后能更准确地判断用药安全。”），空过敏史不阻断查药（SAFE + `safe_without_history` 保留）；②小程序 `finishPillbox`/`finishMedLookup` 按 `hint`/`reminder` 追加文本气泡。验证：mvn 338 tests 全绿、spotless 通过；端到端无档案患者拍药盒响应含 `reminder`、会话 4 条消息（image + medication_info + medication_safety + reminder 文本），文字查药 not_found 响应含 `hint`。待支付宝开发者工具人工走查（可识别图 + not_found 图各一遍）。
