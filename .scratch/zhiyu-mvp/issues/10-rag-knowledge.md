...
- [x] 固定 10 条典型症状查询集及期望知识块，Top-3 命中不少于 8 条方可通过（集成测试连真实 PG+pgvector+方舟，query 向量缓存）- 实测 10/10 命中
...
- ~~`doubao-embedding-vision` 实际维度实现期连 endpoint 探测确认~~：已确认返回 2048 维，DDL/契约/yml 已同步改为 2048。
