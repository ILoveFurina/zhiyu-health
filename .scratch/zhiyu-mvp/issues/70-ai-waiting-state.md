...
- 回归修复二（2026-08-09，真根因）：上一轮修复后流式仍丢失，逐跳实测（server-py SSE、server-java SSE/WS 中继、页面 harness）证明全链路逐 token 正常——真正非流式路径只有端侧 SSE 降级：真机经 cpolar 隧道时 WS 握手必 401（隧道剥 Authorization 头，见 wss-and-windows-service-pitfalls 第 6 节），降级后 `my.request` 整段响应一次性到达、全部事件在一个 JS tick 内同步派发，气泡从等待态直跳全文。修复：`chat-stream.js` SSE 降级改为按节奏重放（token 20ms/片、thinking 8ms/片，对齐实测 LLM 流速），其余事件按序即时下发；轮次终结（done/error/页面卸载置空 current）即停，迟到 token 不覆盖终态。诊断与修复全程记录于 `docs/engineering-notes/sse-fallback-streaming-replay.md`。
