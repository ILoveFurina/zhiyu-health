...
- 2026-07-30：经用户授权执行 `deploy/neo4j/seed.cypher`（30 药品节点 / 9 禁忌关系 / 8 相互作用关系），随后全路径复验通过：氯雷他定[3] → SAFE；阿莫西林[1] → BLOCKED（过敏史“青霉素”×成分匹配）；布洛芬[2]+华法林[4] → BLOCKED（相互作用“合用可能增加胃肠道及其他出血风险”）；绕过前端直接提交[1] → 409（`blocked_prescription` 话术+原因）。浏览器复验真实过敏 BLOCKED 红字警告与提交禁用，控制台零错误。遗留观察（非缺陷，后续可打磨）：B 端实时检查提示沿用契约 C 端话术“已阻止本次药品推荐”，B 端专属提示文案可在后续票评估；409 拒绝话术已为 B 端专属。
