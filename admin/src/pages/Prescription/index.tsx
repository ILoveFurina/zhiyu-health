...
import { fetchPrescriptions, reviewPrescription, type Prescription } from '@/services/prescription';
...
export default function PrescriptionPage() {
  ...
  const load = useCallback(() => fetchPrescriptions('PENDING').then(setRows), []);
  useEffect(() => { load().catch(() => {}); }, [load]);

  const review = async (row: Prescription, decision: 'APPROVE' | 'REJECT', rejectReason?: string) => {
    await reviewPrescription(row.id, decision, rejectReason);
    message.success(decision === 'APPROVE' ? '审核已通过，患者现可查看' : '已驳回电子处方');
  ...
  const columns: TableColumnsType<Prescription> = [
    ...
    { title: '操作', width: 160, render: (_, row) => <Space>
      <Button type="link" onClick={() => review(row, 'APPROVE')}>通过</Button>
  ...
  return <PageContainer title="电子处方审核">
    ...
    <Modal title="驳回电子处方" open={!!rejecting} okButtonProps={{ danger: true, disabled: !reason.trim() }}
      onCancel={() => setRejecting(undefined)} onOk={() => rejecting && review(rejecting, 'REJECT', reason)}>
