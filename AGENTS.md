...
3. PostgreSQL 存业务实体；Neo4j 只存症状、疾病、科室、药品、禁忌等医学知识；MinIO 旁路存储图片对象：拍照分析原图、医生头像与在线问诊交流图片（ADR-0023、ADR-0029）；禁止双写。server-py 对 pgvector 只读，Redis 号源计数仅由 server-java 操作。图谱在线写入（票 89 起）只经 server-java `service/knowledge/GraphAdminService` 写 seam，白名单见 `contracts/graph-management.json`；server-py 对 Neo4j 保持只读。
