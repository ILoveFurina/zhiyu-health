...
7. **写注释**：仅在复杂逻辑、业务规则或特殊处理处添加注释。注释精简，应解释“为什么这么做”，而不是“代码做了什么”。事务边界、并发锁、跨存储补偿、原子操作与非直观 SQL 属于必须注释的复杂逻辑，须在关键决策点就地说明所防止的竞态或失败模式。每票交付前必须审阅本票 diff：若上述逻辑无法仅凭代码与就地注释回答“为什么这样设计、失败时如何保持一致”，视为未完成。
...
工程碰壁记录统一放在 `docs/retrospectives/`。开始一张票前，按票号、领域和标签检查索引中的相关复盘；出现架构落层返工、安全边界遗漏、幂等语义错误、错误的验证结论或多轮审查反复时，交付前按模板新增或更新复盘。

复盘是经验和开工检查项，不自动成为硬约束。需要长期强制执行的结论必须另行更新本文件或新增 ADR。复盘不得复制患者敏感原文、密钥、完整会话流水或无证据的归因。
